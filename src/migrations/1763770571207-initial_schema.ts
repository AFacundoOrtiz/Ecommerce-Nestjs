...
export class InitialSchema1763770571207 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`categories\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(50) NOT NULL, UNIQUE INDEX \`IDX_8b0be371d28245da6e4f4b6187\` (\`name\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`products\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(50) NOT NULL, \`description\` text NOT NULL, \`price\` decimal(10,2) NOT NULL, \`stock\` int NOT NULL, \`imgUrl\` varchar(255) NOT NULL DEFAULT 'https://via.placehold.co/400', \`categoryId\` varchar(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`order_details\` (\`id\` varchar(36) NOT NULL, \`price\` decimal(10,2) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`orders\` (\`id\` varchar(36) NOT NULL, \`date\` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP, \`userId\` varchar(36) NULL, \`orderDetailId\` varchar(36) NULL, UNIQUE INDEX \`REL_749e30f71cc0d2d95f8546f459\` (\`orderDetailId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`roles\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`users\` (\`id\` varchar(36) NOT NULL, \`name\` varchar(50) NOT NULL, \`email\` varchar(50) NOT NULL, \`password\` varchar(255) NOT NULL, \`phone\` bigint NULL, \`country\` varchar(50) NULL, \`address\` text NULL, \`city\` varchar(50) NULL, UNIQUE INDEX \`IDX_97672ac88f789774dd47f7c8be\` (\`email\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`order_details_products_products\` (\`orderDetailsId\` varchar(36) NOT NULL, \`productsId\` varchar(36) NOT NULL, INDEX \`IDX_35bbcf9515eab2382bd417b385\` (\`orderDetailsId\`), INDEX \`IDX_df657e601f53f706e4b7d253c3\` (\`productsId\`), PRIMARY KEY (\`orderDetailsId\`, \`productsId\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`order_details_products_products\` DROP FOREIGN KEY \`FK_df657e601f53f706e4b7d253c30\``);
        await queryRunner.query(`ALTER TABLE \`order_details_products_products\` DROP FOREIGN KEY \`FK_35bbcf9515eab2382bd417b385f\``);
        await queryRunner.query(`ALTER TABLE \`orders\` DROP FOREIGN KEY \`FK_749e30f71cc0d2d95f8546f4592\``);
        ...
        await queryRunner.query(`DROP INDEX \`IDX_df657e601f53f706e4b7d253c3\` ON \`order_details_products_products\``);
        await queryRunner.query(`DROP INDEX \`IDX_35bbcf9515eab2382bd417b385\` ON \`order_details_products_products\``);
